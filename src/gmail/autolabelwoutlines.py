...
#            logger.info(f"Mime message: {mime_message}")
            ...
            try:
                ...
                logger.error(f"Error parsing HTML: {e}:{body}")
 ...
 #               # Truncate the body to the first 10000 words 
  #              words = body.split()
   #             body = ' '.join(words[:2000])
    ...
    async def process_and_label_emails(self, training_file:str) -> list[dict]:
        """
        ...
        ollama_api_url = "http://localhost:9001/engines/llama.cpp/v1/chat/completions"
        try:
            ...
            for email in unread_emails:
                ...
                # Write email content and response to file for training
                ...
                logger.info(f"Answer from Ollama: {label}")
                    ...
                    "messages":[{
                        ...
                        "content": "'''# The following contents represent the email formatted as JSON: "+
                           ...
                           +     "[START INSTRUCTIONS] "
                                ...
                                "if email addresses me irectly as JP or Jeffrey then respond with 'Review' "
                                "if the email is a non-political newsletter, respond with 'Ads' "
...
async def explicit(creds_file_path: str,
    ...
    gmail_service = GmailService(creds_file_path, token_path)
    server = Server("gmail")
    results = await gmail_service.process_and_label_emails(training_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--training-file', required=False, help="File to store training data")
